...
def resources_pos_symbol(resources, waters, woods, foods, golds, width_re_ls, height_re_ls, symbol_re):
  ...
  return width_re_ls, height_re_ls, symbol_re
#------------------------------------------------------------------------
#display the map with resources
def map_with_re_lines(total_ls, width_ls, height_ls, symbol_re):
  for u in range(len(width_ls)):
    change_line = total_ls[height_ls[u]]
    change_l_after = change_line[:3*width_ls[u]+3] + symbol_re + change_line[3*width_ls[u]+5:]
...
def match_army(user_input, army_type):
  if user_input == 'S':
    army_type = S
  elif user_input == 'A':
    army_type = A
...
def apply_position(map_without_any_ls, tot_soi_type_and_pos, resource_symbol, resources_ls, stop_num, player_num, map_ls):
...
def check_posin(army_placein_ls, soi_type_and_pos, next_to_base):
  posin_vaild = True
  army_width = int(army_placein_ls[0])
  army_height = int(army_placein_ls[1])
...
def army_will_move(start_move, end_move, soi_type_and_pos, one_round_armies, army_name):
...
def move_to_resource(start_move, end_move, resources_ls, resources_hold, scout_die_between, resource_name):
...
def move_to_kill_other_army(player, end_move, army_name, soi_type_and_pos, soi_type_and_pos_enemy, counter_ls):
  copy_ls_self = soi_type_and_pos[:]
  copy_ls_enemy = soi_type_and_pos_enemy[:]
...
def move_to_be_killed_army(player, end_move, army_name, soi_type_and_pos, soi_type_and_pos_enemy, counter_ls):
  copy_ls_self = soi_type_and_pos[:]
  copy_ls_enemy = soi_type_and_pos_enemy[:]
...
if __name__ == "__main__":
  if len(sys.argv) != 2:
    ...
    sys.exit()
  width, height, waters, woods, foods, golds, bases, resources_ls= load_config_file(sys.argv[1])
...
#Step 3: display the map
  ...
  width_re_ls, height_re_ls, symbol = resources_pos_symbol(waters, waters, woods, foods, golds, width_re_ls, height_re_ls, symbol_re)
  re_total_ls = map_with_re_lines(re_total_ls, width_re_ls, height_re_ls, symbol_re)
  width_re_ls, height_re_ls, symbol = resources_pos_symbol(woods, waters, woods, foods, golds, width_re_ls, height_re_ls, symbol_re)
  re_total_ls = map_with_re_lines(re_total_ls, width_re_ls, height_re_ls, symbol_re)
  width_re_ls, height_re_ls, symbol = resources_pos_symbol(foods, waters, woods, foods, golds, width_re_ls, height_re_ls, symbol_re)
  re_total_ls = map_with_re_lines(re_total_ls, width_re_ls, height_re_ls, symbol_re)
  width_re_ls, height_re_ls, symbol = resources_pos_symbol(golds, waters, woods, foods, golds, width_re_ls, height_re_ls, symbol_re)
  re_total_ls = map_with_re_lines(re_total_ls, width_re_ls, height_re_ls, symbol_re)
...
#5 - d - iii (edge cases 2/3/4/5)
        if army_typein == 'QUIT':
          ...
          print(first_l)
          print(second_l)
          map_ls = apply_position(map_without_any_ls, tot_soi_type_and_pos, resource_symbol, resources_ls, stop_num, player_num, map_ls)
...
#5 - d - i (positive cases)
          ...
          army_ls = match_army(army_typein, army_type)
...
#5 - d - i- 3 (a)
                posin_vaild = check_posin(army_placein_ls, soi_type_and_pos, next_to_base)
...
#positive cases (army have move):
            soi_type_and_pos, army_name, one_round_armies = army_will_move(start_move, end_move, soi_type_and_pos, one_round_armies, army_name)
...
#f: move to a resource
            resources_ls, resources_hold = move_to_resource(start_move, end_move, resources_ls, resources_hold, scout_die_between, resource_name)
...
#c: move to a counter enemy (self army disappear)
            soi_type_and_pos, soi_type_and_pos_enemy = move_to_be_killed_army(player, end_move, army_name, soi_type_and_pos, soi_type_and_pos_enemy, counter_ls)
...
#e: move to a counter enemy (enemy disappear)
            soi_type_and_pos, soi_type_and_pos_enemy = move_to_kill_other_army(player, end_move, army_name, soi_type_and_pos, soi_type_and_pos_enemy, counter_ls)
